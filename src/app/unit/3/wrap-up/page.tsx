...
import { unitThreeWrapUp } from "@/lib/content/unit3-wrapup";
import { useLessonProgress } from "@/lib/hooks";
import { cn } from "@/lib/utils";

export default function UnitThreeWrapUpPage() {
  const { progress } = useLessonProgress();
  const lessons = getLessonsByUnit(3);
      ...
      <main className="mx-auto w-full max-w-3xl flex-1 px-4 pb-24 pt-10 sm:px-6">
        <header className="mb-10">
          ...
          <div className="mt-8" role="presentation">
            ...
            <p className="mt-1 font-mono text-[10px] font-semibold uppercase tracking-[0.16em] text-muted">
              Note: This unit currently has one lesson; more will be added later.
            </p>
          </div>
        </header>

        <CalloutBlock variant="challenge" title={unitThreeWrapUp.harness.title}>
          <p className="text-pretty font-serif text-[17px] leading-relaxed text-foreground">
            {unitThreeWrapUp.harness.prompt}
          </p>
        </CalloutBlock>
        ...
        <section className="mt-14" aria-labelledby="summary-heading">
          ...
          <p className="mb-4 max-w-[68ch] font-serif text-sm italic leading-relaxed text-muted">
            The book&rsquo;s own four-part recap of Unit III (pp. 83–86).
          </p>
          <div className="overflow-hidden rounded-[var(--radius)] border border-border bg-panel">
            <ol className="px-5 py-2">
              {unitThreeWrapUp.summary.map((bullet, index) => (
                <li
                  key={index}
        ...
        <section className="mt-14" aria-labelledby="keywords-heading">
          <header className="mb-4 flex items-baseline gap-3">
            ...
            <span className="font-mono text-[11px] font-semibold uppercase tracking-[0.16em] text-muted">
              {unitThreeWrapUp.keyWords.length} to know
            </span>
          </header>
          <p className="mb-4 max-w-[68ch] font-serif text-sm italic leading-relaxed text-muted">
          ...
          <dl className="divide-y divide-border border-y border-border">
            {unitThreeWrapUp.keyWords.map((keyword) => {
        ...
        <section className="mt-14" aria-labelledby="further-reading-heading">
          ...
          <CalloutBlock variant="note" title="Further reading">
            ...
            <div className="flex flex-wrap gap-2">
              {unitThreeWrapUp.furtherReading.topics.map((topicSlug) => {
          ...
          <div className="flex items-center justify-between gap-4 px-5 py-5">
            <span className="min-w-0">
              ...
              <span className="mt-1 block font-serif text-sm italic leading-relaxed text-muted">
                Not yet available — will be added when the unit is complete
              </span>
            </span>
            <ArrowRightIcon className="h-6 w-6 shrink-0 text-muted" />
          </div>
        </Link>
